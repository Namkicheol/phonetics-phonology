# 임용 음운론 서브노트 마스터 — CLAUDE.md (Claude Code)

## Claude Code 1차 진입 문서

Claude Code는 이 `CLAUDE.md`를 진입 문서로 사용한다. Codex는 `AGENTS.md`를 사용한다. 두 파일은 도구 framing만 다르고 섹션·순서·규칙은 동일하다. 새 규칙은 양쪽 모두 갱신한다.

- `AGENTS.md`: Codex용 parallel 진입 문서. 같은 규칙·순서를 유지한다.
...
> 상세 매핑표·예시·위반 트리거는 **`docs/한글용어.md`** 참조. 정책 원본은 전역 `~/.claude/CLAUDE.md`의 "임용 관련 작업: 한국어 용어 규칙" 및 testmaster `docs/한글용어.md`.
...
실행 원칙:
- 사용자가 명시적으로 "백그라운드 에이전트", "subagent", "병렬 에이전트"를 요청한 경우에만 Task 도구로 subagent를 띄운다.
